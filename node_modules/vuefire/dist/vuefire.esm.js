 ...
 */
function rtdbBindAsObject({ vm, key, document, resolve, reject, ops }, extraOptions = DEFAULT_OPTIONS) {
 ...
 * @returns a function to be called to stop listeninng for changes
 */
function rtdbBindAsArray({ vm, key, collection, resolve, reject, ops }, extraOptions = DEFAULT_OPTIONS) {
    const options = Object.assign({}, DEFAULT_OPTIONS, extraOptions);
    const array = [];
...
function updateDataFromDocumentSnapshot({ snapshot, target, path, subs, ops, depth, resolve }, options) {
...
function subscribeToRefs({ subs, refs, target, path, depth, ops, resolve }, options) {
...
function bindCollection({ vm, key, collection, ops, resolve, reject }, extraOptions = DEFAULT_OPTIONS$1) {
 ...
 * @param extraOptions
 */
function bindDocument({ vm, key, document, resolve, reject, ops }, extraOptions = DEFAULT_OPTIONS$1) {
    const options = Object.assign({}, DEFAULT_OPTIONS$1, extraOptions); // fill default values
...
const rtdbPlugin = function rtdbPlugin(Vue, userOptions = defaultOptions) {
    ...
    Vue.prototype[bindName] = function rtdbBind(key, source, userOptions = globalOptions) {
...
const firestorePlugin = function firestorePlugin(Vue, userOptions = defaultOptions$1) {
    const strategies = Vue.config.optionMergeStrategies;
    strategies.firestore = strategies.provide;
    const globalOptions = Object.assign({}, defaultOptions$1, userOptions);
    const { bindName, unbindName } = globalOptions;
    Vue.prototype[bindName] = function firestoreBind(key, ref, userOptions = globalOptions) {
        const options = Object.assign({}, globalOptions, userOptions);
        if (this._firestoreUnbinds[key]) {
            // @ts-ignore
            this[unbindName](key);
        }
        const promise = bind$1(this, key, ref, ops$1, options);
        // @ts-ignore
